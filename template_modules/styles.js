...
export const stylesPlugins = [
	...
	...((isProduction && templateConfig.styles.pxtorem) ? [{
		...
		closeBundle: {
			order: 'pre',
			handler: async () => {
				const cssFiles = globSync(`dist/${isAssets}css/*.css`)
	...
	...((isProduction) ? [{
		...
		closeBundle: {
			order: 'pre',
			handler: async () => {
				const cssFiles = globSync(`dist/${isAssets}css`)
		...
		closeBundle: {
			...
			handler: () => {
				...
				if (templateConfig.fonts.download) {
					let cssCode = fs.readFileSync(`dist/${isAssets}css/webfonts.min.css`, 'utf8')
					cssCode = cssCode.replace(/src:\s*url\(\s*fonts/gi, `src:url(/wp-content/themes/fls-theme/assets/fonts/`)
					fs.writeFileSync(`dist/${isAssets}css/webfonts.min.css`, cssCode, 'utf8');
				}
			}
		}
	}] : []),
	...
	...((isProduction && templateConfig.styles.devfiles) ? [{
		...
		closeBundle: {
			...
			handler: () => {
				const cssFiles = globSync(`dist/${isAssets}css/*.css`)
				if (cssFiles.length) {
					!fs.existsSync(`dist/${isAssets}css/dev`) && templateConfig.styles.codesplit ? fs.mkdirSync(`dist/${isAssets}css/dev`) : null
					cssFiles.forEach(async (cssFile) => {
